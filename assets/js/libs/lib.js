...
function generateData(no_entry, tableConf, callBack, benchMark) {

    //benchMark.pauseTimer(); // resume
    var doneCount = 0,
        remaningCount,
        localCallBack = function(noDone) {
            ...
            if (doneCount === no_entry) {
                callBack && setTimeout(function() {
                    if (benchMark) {
                        benchMark.pauseTimer(); // resume
                        benchMark.pauseTimer(); // Pause and fire the listner
                    }
    ...
    if (maxIteration < no_entry) {
        while (remaningCount >= maxIteration) {
            ...
            setTimeout(function() {
                gendata(maxIteration, tableConf, localCallBack, benchMark);
            }, 1);
            remaningCount -= maxIteration;
        }
    }
    remaningCount && gendata(remaningCount, tableConf, localCallBack, benchMark);
}
...
function gendata(no_entry, tableConf, callBack, benchMark) {
    var i = 0,
        j,
        data = tableConf.data,
        dataLen = data.length,
    ...
    for (i = 0; i < no_entry; i++, dataLen++) {
        ...
        dataStr += (i ? "," : "") + JSON.stringify(row);
    }
    dataStr += "]";

    benchMark && benchMark.pauseTimer(); // resume

    data.push.apply(data, JSON.parse(dataStr));

    benchMark && benchMark.pauseTimer(true); // Pause the timer and don't fire the event
...
Benchmark.prototype = {
    ...
    pauseTimer: function(dontFireListner) {
        var benchT = this,
        ...
        if (benchT._started) {
            if (!benchT._stopped) {
                ...
                if (benchT._paused && pauseListner && !dontFireListner) {
